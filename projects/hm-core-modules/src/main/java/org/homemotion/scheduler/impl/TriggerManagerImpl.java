...
public class TriggerManagerImpl extends
	...
	public TriggerManagerImpl() {
		super(AbstractTrigger.class, "runtime/Triggers");
		load();
	}
			...
			for (Row row : section) {
				...
				try {
					Constructor<AbstractTrigger> constr = (Constructor<AbstractTrigger>) Class
							.forName(row.get("class")).getConstructor(String.class);
					...
					if(synchVal!=null){
						trigger.setExecuteSynchronized(Boolean.parseBoolean(synchVal));
					}
