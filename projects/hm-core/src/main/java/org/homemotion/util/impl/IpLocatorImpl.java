...
import org.homemotion.util.IpLocation;
import org.homemotion.util.IpLocator;
...
public class IpLocatorImpl implements IpLocator {
	...
	private float stringToFloat(String fString) {
		try {
	 ...
	 * @throws IOException
	 */
	private List<String> getContent(URL url) throws IOException {
		...
		try {
			...
			if (code != 200)
				throw new IOException(
						"IP Locator failed to get the location. Http Status code : "
								+ code + " [" + url + "]");
	...
	private List<String> getContent(HttpURLConnection connection)
			throws IOException {
	...
	public static void main(String args[]) {
		try {
			...
			final IpLocation ipl = new IpLocatorImpl().locate("12.215.42.19");
			...
			System.err.println(Thread.currentThread().getName() + " FEHLER - "
					+ e);
			e.printStackTrace();
		}
	}
